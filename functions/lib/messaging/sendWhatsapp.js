...
const msg91Adapter_1 = require("../providers/messaging/msg91Adapter");
const msg91 = new msg91Adapter_1.Msg91Adapter();
// Hardcoded cost for now, can be fetched from config
const WHATSATSAPP_COST = 1.0;
exports.sendWhatsapp = functions.https.onCall(async (request) => {
    // 1. Auth Check
    if (!request.auth) {
    ...
    const leadRef = firebase_1.db.doc(`tenants/${tenantId}/leads/${leadId}`);
    ...
    return await firebase_1.db.runTransaction(async (transaction) => {
        ...
        try {
            ...
            await firebase_1.db.doc(`tenants/${tenantId}/interactions/${interactionId}`).update({
