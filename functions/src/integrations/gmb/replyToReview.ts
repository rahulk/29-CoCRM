...
export const replyToReview = functions.https.onCall(async (request) => {
    ...
    if (!replyText) throw new functions.https.HttpsError("invalid-argument", "Reply text required");

    // Fetch tokens
    const docSnap = await db.doc(`tenants/${tenantId}/integrations/gmb`).get();
    ...
    try {
        ...
        const reviewDoc = await db.doc(`tenants/${tenantId}/reviews/${reviewId}`).get();
        const reviewData = reviewDoc.data();

        if (!reviewData) throw new functions.https.HttpsError("not-found", "Review not found in DB");
