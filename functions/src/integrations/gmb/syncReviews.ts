...
async function getClientForTenant(tenantId: string) {
    const doc = await db.doc(`tenants/${tenantId}/integrations/gmb`).get();
    const data = doc.data();
...
export const syncReviews = functions.https.onCall(async (request) => {
    // ... auth checks ...
    ...
    if (!gmbData) return { success: false, reason: "Not connected or no locations" };
    ...
    for (const location of locations) {
        ...
        try {
            ...
            for (const review of reviews) {
                ...
                const reviewRef = db.doc(`tenants/${tenantId}/reviews/${reviewId}`);
