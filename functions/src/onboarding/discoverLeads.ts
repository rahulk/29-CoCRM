...
export const discoverLeads = onCall({ region: "asia-south1", secrets: ["GOOGLE_MAPS_API_KEY"] }, async (request) => {
    ...
    if (!keyword || keyword.trim().length === 0) {
        throw new HttpsError("invalid-argument", "Search keyword is required.");
    }
    ...
    await checkRateLimit(db, `tenant_${tenant_id}_discoverLeads`, RATE_LIMITS.DISCOVER_LEADS.maxCalls, RATE_LIMITS.DISCOVER_LEADS.windowMs);
    ...
    if (isPreview) {
        ...
        if ((tenantData.usage_current?.preview_searches_used || 0) >= 3) {
            throw new HttpsError("resource-exhausted", "Preview search limit reached. Start trial to continue.");
        }
    } else {
        ...
        if (currentUsage >= maxLimit) {
            throw new HttpsError("resource-exhausted", "Monthly lead limit reached. Upgrade plan.");
        }
    }
    ...
    try {
        ...
        throw new HttpsError("unavailable", "Google Places API unavailable.");
    }
    ...
    const leadIds = places.map((p: any) => {
        const rawId = tenant_id + (p.id || p.formattedAddress);
        return crypto.createHash('sha256').update(rawId).digest('hex');
    });

    const existingDocs = await db.getAll(...leadIds.map((id: string) => db.collection("leads").doc(id)));
    ...
    for (let i = 0; i < places.length; i++) {
        ...
        if (isPreview && limit === 5 && savedCount >= 5) break;
        ...
        batch.set(db.collection("leads").doc(leadId), leadData);
        savedCount++;
    }
    ...
    if (savedCount > 0) {
        ...
        if (isPreview) {
            updateData['usage_current.preview_searches_used'] = FieldValue.increment(1);
        } else {
            updateData['usage_current.leads_fetched_this_month'] = FieldValue.increment(savedCount);
        }
